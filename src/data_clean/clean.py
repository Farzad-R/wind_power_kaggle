...
class Clean:
    def clean_train():
        ...
        df = pd.read_csv(here(config["clean_train"]["input"]))
        df = df.index.resample("1h")
        # check the output and save the file
        EnvUtils.check_directory(here(config["clean_train"]["output"]))
        output_path = os.path.join(
            here(),
            config["clean_train"]["output"],
            config["clean_train"]["output_filename"],
        )
        print(output_path)
...
if __name__ == "__main__":
    Clean.clean_train()
    # Clean.transform_test()
    # Clean.transform_wind()
